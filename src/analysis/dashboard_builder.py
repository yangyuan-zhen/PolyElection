...
def _normalize_title(title: str) -> str:
    ...
    patterns = [
        (r"^Democratic Presidential Nominee (\d{4})$", lambda m: f"{m.group(1)}年民主党总统候选人"),
        (r"^Republican Presidential Nominee (\d{4})$", lambda m: f"{m.group(1)}年共和党总统候选人"),
        (r"^Presidential Election Winner (\d{4})$", lambda m: f"{m.group(1)}年总统选举获胜者"),
        (r"^Next Prime Minister of (.+)$", lambda m: f"{_translate_place(m.group(1))}下一任总理"),
        (r"^(.+?) Republican Senate Primary Winner$", lambda m: f"{_translate_place(m.group(1))}共和党参议院初选获胜者"),
        (r"^(.+?) Democratic Senate Primary Winner$", lambda m: f"{_translate_place(m.group(1))}民主党参议院初选获胜者"),
        (r"^(.+?) Senate Election Winner$", lambda m: f"{_translate_place(m.group(1))}参议院选举获胜者"),
        (r"^(.+?) Chamber of Representatives Election Winner$", lambda m: f"{_translate_place(m.group(1))}众议院选举获胜者"),
        (r"^(.+?) Presidential Election$", lambda m: f"{_translate_place(m.group(1))}总统选举"),
        (r"^(.+?) Prime Minister$", lambda m: f"{_translate_place(m.group(1))}总理"),
        (r"^(.+?) Parliamentary$", lambda m: f"{_translate_place(m.group(1))}议会选举"),
        (r"^(.+?) Legislative$", lambda m: f"{_translate_place(m.group(1))}立法选举"),
        (r"^(.+?) Mayoral$", lambda m: f"{_translate_place(m.group(1))}市长选举"),
        (r"^(.+?) Mayor$", lambda m: f"{_translate_place(m.group(1))}市长选举"),
        (r"^(.+?) Regional Election$", lambda m: f"{_translate_place(m.group(1))}地区选举"),
    ]
...
def _normalize_question(question: str, title_zh: str, outcome_zh: str) -> str:
    translated = _replace_from_map(question.strip(), PLACE_TRANSLATIONS)
    translated = _replace_from_map(translated, PHRASE_TRANSLATIONS)
    translated = _replace_from_map(translated, OUTCOME_TRANSLATIONS)
    translated = re.sub(r"\s+", " ", translated).strip()
    if not translated:
        return f"该市场围绕“{title_zh}”展开，当前主导结果为“{outcome_zh}”。"
    if translated == question.strip():
        return f"该市场围绕“{title_zh}”展开，当前主导结果为“{outcome_zh}”。"
...
def _build_description(title_zh: str, question_zh: str, end_date: Optional[datetime]) -> str:
    if end_date is None:
        return f"{question_zh} 当前市场焦点围绕“{title_zh}”，结算时间待定。"
    ...
    pairs = [
        {
            ...
            "label_zh": _translate_outcome(outcomes[index] if index < len(outcomes) else f"Outcome {index + 1}"),
            "price": price,
        }
...
def _pick_market_probability(market: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if set(normalized_outcomes) == {"yes", "no"}:
        chosen = next((pair for pair in valid_pairs if pair["label"].strip().lower() == "yes"), valid_pairs[0])
    else:
        chosen = max(valid_pairs, key=lambda item: item["price"])
    volume = _coerce_float(market.get("volume24hr") or market.get("volume24Hr") or market.get("volumeNum") or market.get("volume"))
...
def _infer_candidate_party_label(candidate_name: str, title: str, market: Optional[Dict[str, Any]] = None) -> str:
    source = " ".join([
        title or "",
    ...
    for token, label in PARTY_LABEL_MAP:
        if token in source:
            return label
...
def _build_candidate_board(markets: Iterable[Dict[str, Any]], primary_market: Optional[Dict[str, Any]], title: str) -> List[Dict[str, Any]]:
    if primary_market:
            ...
            return [
                {
                    ...
                    "party_label": _infer_candidate_party_label(item["label"], title, primary_market),
                    "image": primary_market.get("image") or primary_market.get("icon") or primary_market.get("twitterCardImage"),
                }
    ...
    for market in markets:
        ...
        if normalized != {"yes", "no"}:
            continue
        yes_outcome = next((item for item in market_outcomes if item["label"].strip().lower() == "yes"), None)
        ...
        contenders.append(
            {
                ...
                "party_label": _infer_candidate_party_label(contender_label, title, market),
                "image": market.get("image") or market.get("icon") or market.get("twitterCardImage"),
            }
        )
    deduped: Dict[str, Dict[str, Any]] = {}
    for contender in contenders:
        deduped[contender["name"].lower()] = contender
    ordered = sorted(deduped.values(), key=lambda item: item["probability"], reverse=True)
    return ordered[:8]



def _select_focus_selection(selections: List[Dict[str, Any]], candidate_board: List[Dict[str, Any]]) -> Dict[str, Any]:
    default_primary = max(selections, key=lambda item: (item["volume"], item["liquidity"], item["price"]))
...
def _build_opportunity(event: Dict[str, Any], now: datetime) -> Optional[Dict[str, Any]]:
    ...
    days_left = max((event_end.date() - now.date()).days, 0) if event_end and event_end > now else 0
    volume_24h = _coerce_float(event.get("volume24hr") or event.get("volume24Hr") or event.get("volume"))
    ...
    return {
        ...
        "peb_prob": round(market_probability * 100, 1),
        "market_blend_prob": round(market_probability * 100, 1),
        "market_cross_section_prob": round(blended_probability * 100, 1),
...
def _build_intelligence(opportunities: List[Dict[str, Any]], now: datetime) -> List[Dict[str, Any]]:
    ...
    sorted_by_time = sorted(opportunities, key=lambda item: item["end_date"] or "9999-12-31T00:00:00+00:00")
    sorted_by_divergence = sorted(opportunities, key=lambda item: abs(item["divergence"]), reverse=True)
    feed = [
        {
            "tag": "24小时成交额",
            "title": sorted_by_volume[0]["title_zh"],
            "impact": f"{sorted_by_volume[0]['outcome_label_zh']} 当前领先，主市场概率 {sorted_by_volume[0]['market_price']:.1f}%，24小时成交额 {sorted_by_volume[0]['volume_24h_label']}。",
            "time": "实时",
        },
        {
            "tag": "流动性",
            "title": sorted_by_liquidity[0]["title_zh"],
            "impact": f"流动性达到 {sorted_by_liquidity[0]['liquidity_label']}，当前主导结果为 {sorted_by_liquidity[0]['outcome_label_zh']}。",
            "time": "实时",
        },
        {
            "tag": "价差",
            "title": sorted_by_divergence[0]["title_zh"],
            "impact": f"融合概率 {sorted_by_divergence[0]['peb_prob']:.1f}%，主市场概率 {sorted_by_divergence[0]['market_price']:.1f}%。",
    ...
    if earliest_end and earliest_end > now:
        feed[2] = {
            ...
            "impact": f"预计在 {earliest_end.date().isoformat()} 结算，当前领先结果为 {earliest['outcome_label_zh']}，主市场概率 {earliest['market_price']:.1f}%。",
...
def build_dashboard_payload(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    opportunities.sort(
        key=lambda item: (item["volume_24h"], item["liquidity"], item["market_price"]),
        reverse=True,
    )
...
def build_dashboard_sections(opportunities: List[Dict[str, Any]], *, now: Optional[datetime] = None) -> Dict[str, Any]:
